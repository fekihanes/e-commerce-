<?php 
 ...
 $mode = isset($_GET['mode']) ? mysqli_real_escape_string($con, $_GET['mode']) : '';
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<?php include "navbar.php"?>
    ...
    <section>
        ...
        if($mode=="low_to_high" ){
            $result = mysqli_query($con, "SELECT * FROM manga order by prix ");                     
        }elseif ($mode =="high_to_low") {
              $result = mysqli_query($con, "SELECT * FROM manga order by  prix desc");                     
              
        } elseif ($mode =="a-z" ) {
              $result = mysqli_query($con, "SELECT * FROM manga order by nom");                     
              
        } elseif ($mode=="z-a") {
            $result = mysqli_query($con, "SELECT * FROM manga order by nom desc");                     

        } elseif ($mode=="nouvel_article") {
            $result = mysqli_query($con, "SELECT * FROM manga  where date=CURDATE()");                     

        }elseif ($mode=="top_article") {

            $result = mysqli_query($con, "SELECT * FROM manga  where id in (select id_product from rate group by (id_product) having avg(note)>4)");                     

        }
        elseif ($mode=="to_popularity") {
            $result = mysqli_query($con, "SELECT m.* FROM manga m left join rate r on r.id_product = m.id group by m.id order by avg(r.note) desc");
        }
        ...
        ?>         
            ...
            <a href="more_info.php?id=<?=$row['id']?>">
              ...
              <div><h4 style="margin-left: 20px ">prix :  
                  ...
                  <a class="delete_btn" href="add_panier.php?id=<?=$row['id']?>&loca=filter.php?mode=<?=$mode?>">
